...
try {
	...
	$Tickets = new Tickets();
	$ticketsList = $Tickets->getAwaitingResponseTickets();
	if(!is_array($ticketsList)) throw new Exception('There are no tickets awaiting response.');
	
	echo '<div class="card">';
		echo '<div class="header">Awaiting Response: <span class="text-info">'.number_format(count($ticketsList)).'</span></div>';
